...
function updateAdvisor() {
    ...
    // Show up to 4 tips — compact single-line with colored prefix
    const shown = tips.slice(0, 4);
    for (let i = 0; i < shown.length; i++) {
        const tip = shown[i];
        const row = document.createElement('div');
        row.style.cssText = 'padding:3px 0; font-size:0.78em; line-height:1.35;'
            + (i < shown.length - 1 ? ' border-bottom:1px solid rgba(255,255,255,0.04);' : '');

        const catColor = tip.urgent ? '#ff6b6b'
            : tip.cat === 'Next Step' ? '#33ffdd'
...
function updateCampStatus() {
    ...
    // Only show stats NOT already in the HUD (Pop, Workers, Knowledge are in HUD)
